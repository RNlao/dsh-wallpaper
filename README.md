...
- **文字可读性**：文字颜色（自动/浅色/深色，仅作用于聊天内容）+ 文字阴影 + **对比保护**（关闭/均衡/增强，为思考块与输入提示加主题自适应的底色）。
...
- **文字颜色与对比保护**：文字颜色只注入聊天会话子树（`[data-slot="conversation.session"]`），不泄漏到设置/轨迹/侧边栏等不透明表面；对比保护给思考块（`[data-variant="think"]`）与输入提示（`[data-decoration="hint"]`）加主题自适应的半透明底色。

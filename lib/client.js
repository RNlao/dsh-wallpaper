...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // Surface tokens intentionally preserve DSH's nested frame/workspace/sidebar
    // depth; performance optimizations only avoid redundant image and style work.
    var SURFACE_TOKENS = {
    ...
    function apply(ctx) {
      ...
      function buildCss(c, urls) {
        ...
        var mainTokens = buildTokens(main);
        var surfaceCss = [];
        if (Object.keys(mainTokens).length > 0) {
          var frameLight = {};
          var frameDark = {};
          Object.keys(mainTokens).forEach(function (token) {
            frameLight[token] = mainTokens[token].light;
            frameDark[token] = mainTokens[token].dark;
          });
          surfaceCss.push("body:not([data-ds-dark-theme]) .pI_x6G_frame{" + declarations(frameLight) + "}");
          surfaceCss.push("body[data-ds-dark-theme] .pI_x6G_frame{" + declarations(frameDark) + "}");
        }
      ...
      function revokeActiveUrls() {
        Object.keys(imageUrls).forEach(function (id) { URL.revokeObjectURL(imageUrls[id]); });
        imageUrls = {};
      ...
      async function applyWallpaper(c) {
        ...
        var missingIds = imageIds.filter(function (id) { return !imageUrls[id]; });
        var records = {};
        if (missingIds.length > 0) {
          ...
          missingIds.forEach(function (id) {
            if (records[id] && records[id].blob) imageUrls[id] = URL.createObjectURL(records[id].blob);
          });
        }
        if (seq !== applySeq) return;

        var next = Object.assign({}, c);
        if (main.mode === "image" && !imageUrls[main.activeId]) next = Object.assign(next, { mode: "none", activeId: null });
        if (trajectory.mode === "image" && !imageUrls[trajectory.activeId] && c.trajectoryMode !== "inherit") next = Object.assign(next, { trajectoryMode: "none", trajectoryActiveId: null });
        if (next !== c) {
          cfg = next;
          safeSet(STORAGE_KEY, JSON.stringify(next));
        }

        var nextMain = paneConfig(next, "main");
        var nextTrajectory = paneConfig(next, "trajectory");
        var keepIds = {};
        [nextMain, nextTrajectory].forEach(function (pane) {
          if (pane.mode === "image" && pane.activeId && imageUrls[pane.activeId]) keepIds[pane.activeId] = true;
        });
        ...
        activeUrls = {
          main: nextMain.mode === "image" ? imageUrls[nextMain.activeId] || null : null,
          trajectory: nextTrajectory.mode === "image" ? imageUrls[nextTrajectory.activeId] || null : null
        };
      ...
      ctx.effect(function () {
        return function () {
          if (applyFrame !== null) { cancelAnimationFrame(applyFrame); applyFrame = null; }
          applySeq += 1;
          if (disposeStyle) { disposeStyle(); disposeStyle = null; }
          revokeActiveUrls();
        };

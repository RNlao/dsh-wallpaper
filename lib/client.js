// ============================================================================
// dsh-wallpaper · 客户端半 (browser half) —— 静态插件格式 v3
// ----------------------------------------------------------------------------
// 图片作为真实文件存到磁盘（$DSH_HOME/wallpaper/），通过 host 半的 HTTP 路由存取；
// localStorage 只保存「当前选择 + 效果参数」，不再存图片本身。
// ============================================================================
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var STORAGE_KEY = "dsh-wallpaper:v3";
    ...
    // ---- host 半 HTTP API ----
    function apiList() {
      return fetch(API + "/list").then(function (r) { return r.json(); });
    }
    function apiSave(name, dataUrl) {
    ...
    function apiDelete(id) {
      return fetch(API + "/delete?id=" + encodeURIComponent(id)).then(function (r) { return r.json(); });
    }
    function fileUrl(id) {
    ...
    function apply(ctx) {
      ...
      function bgValue(c) {
        if (c.mode === "image" && c.activeId) return 'url("' + fileUrl(c.activeId) + '")';
      ...
      function WallpaperSection() {
        ...
        var refreshGallery = function () {
          apiList().then(function (res) {
            if (res && res.ok) setGallery(res.files || []);
          }).catch(function () {});
        };
        ...
        var saveAndActivate = function (nm, dataUrl) {
          setBusy(true);
          return apiSave(nm, dataUrl).then(function (res) {
            setBusy(false);
            if (res && res.ok) {
              refreshGallery();
              commit({ mode: "image", activeId: res.id });
            } else {
              setNotice((res && res.error) || "保存失败");
            }
          }).catch(function (e) {
            setBusy(false);
            setNotice("保存失败：" + (e && e.message ? e.message : String(e)));
          });
        };
        ...
        var onFile = function (e) {
          ...
          if (!file) return;
          fileToDataUrl(file).then(function (dataUrl) { return saveAndActivate(file.name, dataUrl); });
          e.target.value = "";
        };
        ...
        var onDir = function (e) {
          ...
          var imgs = [];
          if (files) {
            for (var i = 0; i < files.length; i++) if (IMG_EXT_RE.test(files[i].name)) imgs.push(files[i]);
          }
          e.target.value = "";
          if (imgs.length === 0) { setNotice("该目录下没有图片文件。"); return; }
          setBusy(true);
          setNotice("正在导入 " + imgs.length + " 张图片…");
          var firstId = null;
          ...
          async function worker() {
            while (queue.length > 0) {
              ...
              try {
                ...
                var res = await apiSave(f.name, dataUrl);
                if (res && res.ok && firstId === null) firstId = res.id;
              } catch (err) {}
              done++;
            }
          }
        ...
        var onCrop = function (dataUrl) {
          saveAndActivate("裁剪-" + Date.now() + ".jpg", dataUrl).then(function () { setCropping(false); });
        };
        ...
        return h("div", { className: "dshwp-root" },
          ...
          h("section", { className: "dshwp-card" },
            ...
            h("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" } },
              ...
              h("button", { className: "dshwp-btn", onClick: function () { if (dirRef.current) dirRef.current.click(); } }, "导入文件夹"),
              c.mode === "image" && c.activeId
                ? h("button", { className: "dshwp-btn", onClick: function () { setCropping(true); } }, "裁剪当前图片")
                : null
            ),
            busy ? h("div", { className: "dshwp-note", style: { marginTop: 8 } }, notice || "处理中…") : null,
            notice && !busy ? h("div", { className: "dshwp-error", style: { marginTop: 8 } }, notice) : null,
            h("div", { className: "dshwp-note", style: { marginTop: 8 } }, "图片以原图保存到磁盘目录 " + "~/.dsh/wallpaper/（DSH 配置文件夹内），共 " + fmtSize(total) + "。"),
            gallery.length === 0
              ? h("div", { className: "dshwp-note", style: { marginTop: 8 } }, "还没有图片——上传一张，或「导入文件夹」批量加入。")
              : h("div", { className: "dshwp-file-list" },
                  gallery.map(function (g) {
                    return h("div", {
                      ...
                      h("img", { className: "dshwp-file-thumb", src: fileUrl(g.id), alt: g.name }),
          ...
          cropping && c.mode === "image" && c.activeId
            ? h(CropModal, {
                src: fileUrl(c.activeId),
                onApply: onCrop,

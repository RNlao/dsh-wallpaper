  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      function buildCss(c) {
        ...
        // 文字轮廓（描边）+ 阴影：统一为多方向 text-shadow。描边是文字的「对比色
        // 轮廓」，让文字在任意壁纸上都清晰，不再给思考/提示加背景框。
        var protection = c.readability === "strong" ? "strong" : c.readability === "off" ? "off" : "balanced";
        function makeTextFx(outlineRgb, shadowRgb) {
          var parts = [];
          if (protection !== "off") {
            var oa = protection === "strong" ? "0.6" : "0.42";
            var oc = "rgba(" + outlineRgb + "," + oa + ")";
            parts.push(
              "-1px -1px 0 " + oc, "1px -1px 0 " + oc, "-1px 1px 0 " + oc, "1px 1px 0 " + oc,
              "0 1px 0 " + oc, "0 -1px 0 " + oc, "1px 0 0 " + oc, "-1px 0 0 " + oc
            );
          }
          if (c.textShadow) parts.push("0 1px 2px rgba(" + shadowRgb + ",0.55)");
          return parts.length ? "{text-shadow:" + parts.join(",") + ";}" : "";
        }
        var textFxCss = "";
        if (c.textMode === "light") {
          textFxCss = sessionScope + makeTextFx("0,0,0", "0,0,0");
        } else if (c.textMode === "dark") {
          textFxCss = sessionScope + makeTextFx("255,255,255", "255,255,255");
        } else {
          var lightFx = makeTextFx("255,255,255", "255,255,255");
          var darkFx = makeTextFx("0,0,0", "0,0,0");
          if (lightFx) textFxCss += "body:not([data-ds-dark-theme]) " + sessionScope + lightFx;
          if (darkFx) textFxCss += "body[data-ds-dark-theme] " + sessionScope + darkFx;
        }
        return [

...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // 只覆盖三个「大面积背景」token：主画布 + 侧边栏 + 输入框。
    // 内部卡片（bg-layer-* / menu / bubble）保持实底，保证文字可读。
    var SURFACE_TOKENS = {
    ...
    var I18N = {
      zh: {
        language: "界面语言", preview: "预览", noWallpaperCurrent: "当前无壁纸（DSH 原样）",
        presets: "预设", noWallpaper: "无壁纸", library: "图片库", upload: "上传图片",
        importFolder: "导入文件夹", cropCurrent: "裁剪当前图片", processing: "处理中…",
        storage: "图片以原始字节存浏览器 IndexedDB（不压缩、不受 5 MB 限制），共 {size}。",
        emptyLibrary: "还没有图片——上传一张，或「导入文件夹」批量加入。",
        setBackground: "点击设为背景：{name}", remove: "删除", effects: "效果", blur: "模糊",
        dim: "遮罩", opacity: "面板不透明度", readability: "文字可读性", textColor: "文字颜色",
        auto: "自动", light: "浅色", dark: "深色", textShadow: "文字阴影", on: "开", off: "关",
        protection: "对比保护", protectionOff: "关闭", protectionBalanced: "均衡", protectionStrong: "增强",
        readabilityNote: "文字颜色只作用于聊天内容；对比保护会在思考和输入提示后加入主题自适应底色。",
      ...
      en: {
        language: "Language", preview: "Preview", noWallpaperCurrent: "No wallpaper (DSH default)",
        ...
        storage: "Original image bytes are stored in browser IndexedDB (no compression or 5 MB limit). Total: {size}.",
        ...
        dim: "Dim", opacity: "Panel opacity", readability: "Readability", textColor: "Text color",
        auto: "Auto", light: "Light", dark: "Dark", textShadow: "Text shadow", on: "On", off: "Off",
        protection: "Contrast protection", protectionOff: "Off", protectionBalanced: "Balanced", protectionStrong: "Strong",
        readabilityNote: "Text color is scoped to chat content. Contrast protection adds a theme-aware backing to reasoning and input hints.",
    ...
    function apply(ctx) {
      ...
      function buildCss(c) {
        ...
        var composerScope = '[data-conversation-scroll]:not(:has([data-conversation-composer-overlay]))>[data-composer-seat]';
        var textCss = "";
        ...
        var shadowCss = "";
        if (c.textShadow) {
          if (c.textMode === "light") shadowCss = sessionScope + "{text-shadow:0 1px 2px rgba(0,0,0,0.62);}";
          else if (c.textMode === "dark") shadowCss = sessionScope + "{text-shadow:0 1px 2px rgba(255,255,255,0.68);}";
          else shadowCss = 'body:not([data-ds-dark-theme]) ' + sessionScope + '{text-shadow:0 1px 2px rgba(255,255,255,0.72);}body[data-ds-dark-theme] ' + sessionScope + '{text-shadow:0 1px 2px rgba(0,0,0,0.68);}';
        }
        var protection = c.readability === "strong" ? "strong" : c.readability === "off" ? "off" : "balanced";
        var protectionCss = "";
        if (protection !== "off") {
          var strength = protection === "strong" ? 82 : 62;
          var protectBg = "color-mix(in srgb,var(--dsw-alias-bg-layer-1) " + strength + "%,transparent)";
          if (c.textMode === "light") protectBg = protection === "strong" ? "rgba(0,0,0,0.68)" : "rgba(0,0,0,0.46)";
          if (c.textMode === "dark") protectBg = protection === "strong" ? "rgba(255,255,255,0.78)" : "rgba(255,255,255,0.56)";
          protectionCss = sessionScope + ' [data-variant="think"]{background:' + protectBg + ';border-radius:6px;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);}' + composerScope + ' [data-decoration="hint"]{background:' + protectBg + ';border-radius:2px;box-shadow:0 0 0 2px ' + protectBg + ';box-decoration-break:clone;-webkit-box-decoration-break:clone;}';
        }
        return [
      ...
      function WallpaperSection() {
        ...
        return h("div", { className: "dshwp-root" },
          ...
          h("section", { className: "dshwp-card" },
            ...
            h("div", { className: "dshwp-note", style: { marginTop: 8 } }, t("storage", { size: fmtSize(total) })),
            gallery.length === 0
          ...
          h("section", { className: "dshwp-card" },
            ...
            h("div", { className: "dshwp-row" },
              h("label", null, t("protection")),
              h("div", { className: "dshwp-segment" },
                [["off", "protectionOff"], ["balanced", "protectionBalanced"], ["strong", "protectionStrong"]].map(function (item) {
                  return h("button", {
                    key: item[0],
                    className: "dshwp-btn" + (c.readability === item[0] ? " dshwp-active" : ""),
                    "aria-pressed": c.readability === item[0],
                    onClick: function () { commit({ readability: item[0] }); }
                  }, t(item[1]));
                })
              )
            ),

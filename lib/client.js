...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // 需要转成半透明的「表面」token（色值与 design-platform.css 的 --dsw-static-* 一致）。
    // kind 决定透明度档位：base 最透（让壁纸透出），input/solid 稍实（保证输入框与选中态可辨）。
    var SURFACE_TOKENS = {
    ...
    // 文字颜色（用户显式选择时覆盖 label token；auto 则跟随主题不动）。
    var TEXT_COLORS = {
      light: {
    ...
    function apply(ctx) {
      ...
      function buildTokens(c) {
        var op = Math.max(0.25, Math.min(1, Number(c.opacity) || 0.85));
        ...
        var tokens = {};
        for (var name in SURFACE_TOKENS) {
          var t = SURFACE_TOKENS[name];
          var a = alphaOf(t.kind);
          tokens[name] = { light: "rgba(" + t.light + "," + a + ")", dark: "rgba(" + t.dark + "," + a + ")" };
        }
        if (c.textMode === "light" || c.textMode === "dark") {
          var cols = TEXT_COLORS[c.textMode];
          for (var label in cols) {
            var rgb = "rgb(" + cols[label] + ")";
            tokens[label] = { light: rgb, dark: rgb };
          }
        }
        return tokens;
      }
      ...
      function applyWallpaper(c) {
        ...
        var tokenKey = String(c.opacity) + "|" + (c.textMode || "auto");
      ...
      function WallpaperSection() {
        ...
        return h("div", { className: "dshwp-root" },
          ...
          h("section", { className: "dshwp-card" },
            ...
            h("div", { className: "dshwp-note" }, "浅色/深色文字会覆盖 DSH 的字体颜色；「自动」跟随主题。文字阴影可增强任意壁纸下的对比度。")
          ),
          // 填充与位置
          h("section", { className: "dshwp-card" },

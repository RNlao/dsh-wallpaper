  ...
  factory: function (require) {
    ...
    var I18N = {
      zh: {
        ...
        protection: "背景模糊", protectionOff: "关闭", protectionBalanced: "轻", protectionStrong: "强",
        readabilityNote: "背景模糊让壁纸在文字后柔化，提升可读性；文字阴影为可选投影。",
    ...
    function apply(ctx) {
      ...
      function buildCss(c) {
        ...
        // 背景毛玻璃：给对话区 / 输入框 / 侧边栏加 backdrop-filter 模糊，让壁纸在
        // 文字与图标后面被柔化，文字无需描边、无需加框即可读。
        var protection = c.readability === "strong" ? "strong" : c.readability === "off" ? "off" : "balanced";
        var blurCss = "";
        if (protection !== "off") {
          var blurPx = protection === "strong" ? "14px" : "8px";
          var glass = "{backdrop-filter:blur(" + blurPx + ");-webkit-backdrop-filter:blur(" + blurPx + ");}";
          blurCss = '[data-conversation-scroll]' + glass + '[data-composer-seat]' + glass + '[data-slot="sidebar.workspaces"]' + glass;
        }
        // 文字阴影（可选投影）
        var shadowCss = "";
        if (c.textShadow) {

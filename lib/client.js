    ...
    function apply(ctx) {
      ...
      function buildCss(c) {
        ...
        return [
          ...
          'div:has(>[data-slot="sidebar.settings"]),div:has(>div>[data-slot="sidebar.settings"]){background:transparent!important;}',
          'div:has(>[data-slot="sidebar.settings"])::before,div:has(>[data-slot="sidebar.settings"])::after,div:has(>div>[data-slot="sidebar.settings"])::before,div:has(>div>[data-slot="sidebar.settings"])::after{background:transparent!important;box-shadow:none!important;}',
          shadowCss
        ].join("");
      }
      ...
      function WallpaperSection() {
        ...
        var onCrop = function (blob) {
          ...
          setCropping(false);
          saveAndActivate(blob, "裁剪-" + Date.now() + ".jpg");
        };
          ...
          h("section", { className: "dshwp-card" },
            ...
            h("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" } },
              ...
              c.mode === "image" && c.activeId
                ? h("button", { className: "dshwp-btn", onClick: startCrop }, "裁剪当前图片")
                : null
            ),
            ...
            h("div", { className: "dshwp-note", style: { marginTop: 8 } }, "图片以原始字节存浏览器 IndexedDB（不压缩、不受 5MB 限制），共 " + fmtSize(total) + "。"),
            gallery.length === 0
